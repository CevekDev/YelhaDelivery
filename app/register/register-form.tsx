...
function SubmitButton() {
  ...
  return (
    <Button type="submit" size="lg" disabled={pending} className="w-full">
      {pending ? 'Envoi du code…' : 'Recevoir mon code par email'}
    </Button>
  );
}
...
function VerifyButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    <Button type="submit" size="lg" disabled={pending || disabled} className="w-full">
      {pending ? 'Vérification…' : 'Vérifier et créer mon compte'}
    </Button>
  );
}
...
export function RegisterForm() {
  ...
  const isVerify = state?.step === 'verify';

  return (
    <Card>
      <CardHeader>
        <CardTitle>{isVerify ? 'Vérifiez votre email' : 'Créer mon compte'}</CardTitle>
      </CardHeader>
      <CardContent>
        <form action={formAction} className="space-y-4" noValidate>
          {/* ── Panneau 1 : informations ──────────────────────────────
              Toujours monté (même caché) pour que les valeurs soient
              renvoyées lors de l'étape de vérification. */}
          <div className={isVerify ? 'hidden' : 'space-y-4'} aria-hidden={isVerify}>
            {/* Restaurant */}
          ...
          {isVerify ? (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Un code à 6 chiffres vient d&apos;être envoyé à votre adresse email. Saisissez-le
                pour finaliser la création de votre compte. Le code expire dans 15 minutes.
              </p>
